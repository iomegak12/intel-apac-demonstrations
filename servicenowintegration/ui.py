...
def main():
    try:
        ...
        st.set_page_config(page_title="Service Now KB Screening Assistant")
        st.title("Service Now KB Screening AI Assistant")
        st.subheader(
            "This AI Assistant would help you to search available and indexed Service Now KBs, that are submitted to the organization!"
        )

        job_descripton = st.text_area("Please paste the 'KB Query' here ...",
                                      key="1",
                                      height=200)

        document_count = st.text_input("No. Of Service Now KB(s) To Return", key="2")
